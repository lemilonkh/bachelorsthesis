...
\begin{document}
  ...
  the results without alwais joining all subject's names and IDs to this dataset. It also facilitates displaying
  ...
  The PageRank algorithm by Page et al. \cite{ilprints422} was originally intended
  to compute the relative likelyhood that a user might arrive at a web page
  ...
  Therefore, the vertex output ("pagerank") of the PageRank algorithm can be interpreted as the
  relative likelihood that a user might arrive at a certain business when freely
  navigating the graph, starting at any connected node.
  The relation output ("weight") can be seen as the probability that the user
  ...
  is reached with a tolerance parameter to keep the algorithm from running endlessly.



  \subsection{Finding important connections }
  \pagebreak
