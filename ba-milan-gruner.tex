...
\begin{document}
  ...
  After the PageRank algorithm was run, the results still have to be interpreted.
  ...
  using an adaptive thresholding technique like Otsu's method, which is normally
  used for image thresholding but can be applied here as well.
  It works by creating a histogram of the absolute probabilities of
  each $pagerank$ value present in the component, which are grouped into bins.
  It then tries to separate the values into two classes and minimize the variance inside these classes.
