...
\begin{document}
  ...
  that only the relevant graph processing sections need to be shown later on.
    ...
    data. The advantages in comparison to the  here (over GraphX) are that arbitrary data types
  ...
  attribute selectors for the source and target nodes. It can be limited by a maximal
  path length, which should be around 3 to not include too complicated paths which
  wouldn't make for useful relations in a simplified graph.
  ...
  this value as previously discussed. Also, all paths of length 0 and 1 have to
  ...
  relations from the PageRank output.
